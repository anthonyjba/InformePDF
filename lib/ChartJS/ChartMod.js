...
(function() {
	...
	helpers.extend(Chart.Controller.prototype, {
		...
		eventHandler: function eventHandler(e) {
		    ...
		    //JJCORPAS - Hover devuelve los elementos activos (en tooltip)!!
			if (this.options.hover.onHoverJJ) {
			    // Linea Original: this.options.hover.onHover.call(this, this.active);
			    this.options.hover.onHoverJJ.call(this, this.tooltipActive, this.lastTooltipActive, e);
			}
		...
		fit: function() {
			...
			if (this.options.ticks.display && this.options.display) {
				...
				if (this.isHorizontal()) {
				    ...
				    //JJ				    
					this.ctx.font = "normal 12px 'Helvetica Neue', Helvetica, Arial, sans - serif";
		...
		update: function(changed) {
			if (this._active.length){
				...
				if (this._options.tooltips.mode === 'single') {
				    ...
				    //JJ Mod
				    var yLabel = yScale ? yScale.getLabelForIndex(element._index, element._datasetIndex) : '',
				    yLabel = yLabel.toLocaleString();
					...
					helpers.each(this._data.datasets, function(dataset, datasetIndex) {
						...
						if (currentElement) {
                            ...
                            //JJ Mod
							var yLabel = yScale ? yScale.getLabelForIndex(currentElement._index, currentElement._datasetIndex) : '';
							yLabel = yLabel.toLocaleString();                                
...
(function() {
	...
	Chart.controllers.bar = Chart.DatasetController.extend({
		...
		setHoverStyle: function(rectangle) {
            ...
            //JJCORPAS
			rectangle._model.backgroundColor = rectangle.custom && rectangle.custom.hoverBackgroundColor ? rectangle.custom.hoverBackgroundColor : helpers.getValueAtIndexOrDefault(dataset.hoverBackgroundColor, index, helpers.color(rectangle._model.backgroundColor).saturate(0.99).darken(0.01).rgbString());
			rectangle._model.borderColor = rectangle.custom && rectangle.custom.hoverBorderColor ? rectangle.custom.hoverBorderColor : helpers.getValueAtIndexOrDefault(dataset.hoverBorderColor, index, helpers.color(rectangle._model.borderColor).saturate(0.99).darken(0.01).rgbString());
...
(function () {
    ...
    Chart.controllers.linearBar = Chart.DatasetController.extend({
        addElements: function () {

            this.getDataset().metaData = this.getDataset().metaData || [];

            helpers.each(this.getDataset().data, function (value, index) {
                this.getDataset().metaData[index] = this.getDataset().metaData[index] || new Chart.elements.LinearRectangle({
                    _chart: this.chart.chart,
                    _datasetIndex: this.index,
                    _index: index,
                });
        ...
        addElementAndReset: function (index) {
            this.getDataset().metaData = this.getDataset().metaData || [];
            var rectangle = new Chart.elements.LinearRectangle({
                _chart: this.chart.chart,
                _datasetIndex: this.index,
                _index: index,
            });
            ...
            // Add to the rectangles array
            this.getDataset().metaData.splice(index, 0, rectangle);
        },

        update: function update(reset) {
            var rectangles = this.getDataset().metaData;
            ...
            // Update Points
            helpers.each(rectangles, function (rectangle, index) {
        ...
        updateElement: function (rectangle, index, reset) {
            ...
            helpers.extend(rectangle, {
                ...
                _model: {
                    ...
                    // Appearance
                    base: this.calculateBarBase(this.index, index),
                    width: rectangle.custom && rectangle.custom.width ? rectangle.custom.width : helpers.getValueAtIndexOrDefault(this.getDataset().width, index, this.chart.options.elements.linearBar.width),
                    backgroundColor: rectangle.custom && rectangle.custom.backgroundColor ? rectangle.custom.backgroundColor : helpers.getValueAtIndexOrDefault(this.getDataset().backgroundColor, index, this.chart.options.elements.linearBar.backgroundColor),
                    borderColor: rectangle.custom && rectangle.custom.borderColor ? rectangle.custom.borderColor : helpers.getValueAtIndexOrDefault(this.getDataset().borderColor, index, this.chart.options.elements.linearBar.borderColor),
                    borderWidth: rectangle.custom && rectangle.custom.borderWidth ? rectangle.custom.borderWidth : helpers.getValueAtIndexOrDefault(this.getDataset().borderWidth, index, this.chart.options.elements.linearBar.borderWidth),

                },
            });

            rectangle._model.skip = rectangle.custom && rectangle.custom.skip ? rectangle.custom.skip : (isNaN(rectangle._model.x) || isNaN(rectangle._model.y));
        ...
        draw: function (ease) {
            ...
            helpers.each(this.getDataset().metaData, function (rectangle, index) {
                rectangle.transition(easingDecimal);
                rectangle.draw();
            }, this);
		...
		ticks: {
			callback: function(tickValue, index, ticks) {
				...
				if (tickValue !== 0) {
					...
					tickString = tickValue.toFixed(numDecimal); //JJ Esto lo transforma en string
					var tickValueConDecimales = parseFloat(tickString); //Conseguimos el n�mero con m�s decimales.
					tickString = tickValueConDecimales.toLocaleString(); //Por �ltimo volvemos a hacer el string
				} else {
...
(function () {
    ...
    Chart.defaults.global.elements.linearBar = {
        backgroundColor: Chart.defaults.global.defaultColor,
        borderWidth: 0,
        borderColor: Chart.defaults.global.defaultColor,
        width: 50
    ...
    Chart.elements.LinearRectangle = Chart.Element.extend({
        draw: function () {
